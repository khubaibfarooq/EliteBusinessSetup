@php
    $services = \App\Models\Service::active()->byCategory('RAKEZ')->get();
@endphp
        ...
        @if($services->count() > 0)
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                @foreach($services as $service)
                    <a href="{{ route('services.show', $service) }}" class="bg-gray-50 rounded-2xl p-6 shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 border border-gray-100 group text-center block">
                        ...
                        <h3 class="font-heading font-bold text-navy text-[13px] md:text-sm uppercase">{{ $service->title }}</h3>
                    </a>
                @endforeach
            </div>
        @else
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                ...
                <a href="{{ $item[2] }}" target="_blank" rel="noopener noreferrer" class="bg-gray-50 rounded-2xl p-6 shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 border border-gray-100 group text-center block">
                    <div class="w-16 h-16 bg-navy rounded-2xl flex items-center justify-center mx-auto mb-4 group-hover:bg-gold transition-colors duration-300">
                        <span class="text-2xl">{{ $item[1] }}</span>
                    </div>
                    <h3 class="font-heading font-bold text-navy text-[13px] md:text-sm uppercase">{{ $item[0] }}</h3>
                </a>
                @endforeach
            </div>
        @endif
